...
int SimpleTexture::init() {
    ...
    //ALOGV("bitImage = %d" , bitImage);
    this->m_texture_id = loadImageTexture();
    ALOGV("texture id = %d" , m_texture_id);

    glClearColor(1.0f, 1.0f, 1.0f, 1.0f);
    return GL_TRUE;
}

void SimpleTexture::resize(int w, int h) {
    this->m_width = w;
    this->m_height = h;
}
...
GLuint SimpleTexture::loadImageTexture() {
    ALOGV("SimpleTexture :: bitImage = %lu" , bitImage);
    if(bitImage == nullptr){
        ALOGE("load texture ERROR");
        return 0;
    }
    ...
    // Load the texture
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, bitImage->width,
                 bitImage->height, 0, GL_RGBA, GL_UNSIGNED_BYTE, bitImage->data);
